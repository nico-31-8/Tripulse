...
export default function PaginaMicrociclo({ params }: { params: Promise<{ id: string }> }) {
  ...
  const crearSesion = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('sesion').insert({
      ...
      estado: 'Planificada',
      usar_cronometro: usarCronometro
    })
    if (error) { setError('Error: ' + error.message) }
    else {
      setDisciplina(''); setFechaSesion(''); setDuracion(''); setRpe(''); setNotas(''); setUsarCronometro(false)
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {mostrarForm && (
          <form onSubmit={crearSesion} className="bg-gray-900 rounded-xl p-6 mb-6 border border-gray-800 flex flex-col gap-4">
            ...
            <div><label className="text-gray-400 text-sm mb-1 block">Fecha de la sesion</label><input type="date" value={fechaSesion} onChange={e => setFechaSesion(e.target.value)} className="bg-gray-800 text-white px-4 py-3 rounded-lg outline-none focus:ring-2 focus:ring-orange-500 w-full" required /></div>
            <input type="number" placeholder="Duracion estimada en minutos (opcional)" value={duracion} onChange={e => setDuracion(e.target.value)} className="bg-gray-800 text-white px-4 py-3 rounded-lg outline-none focus:ring-2 focus:ring-orange-500" />
            <div><label className="text-gray-400 text-sm mb-1 block">RPE estimado (1-10)</label><input type="number" min="1" max="10" placeholder="Ej: 7" value={rpe} onChange={e => setRpe(e.target.value)} className="bg-gray-800 text-white px-4 py-3 rounded-lg outline-none focus:ring-2 focus:ring-orange-500 w-full" /></div>
            ...
            <div className="flex items-center gap-3 bg-gray-800 rounded-lg px-4 py-3">
              <input type="checkbox" id="cronometro" checked={usarCronometro} onChange={e => setUsarCronometro(e.target.checked)} className="w-4 h-4 accent-orange-500" />
              <label htmlFor="cronometro" className="text-white text-sm cursor-pointer">
                Activar cronometro para esta sesion
                <span className="text-gray-400 block text-xs">El atleta podra medir la duracion real de la sesion</span>
              </label>
            </div>
        ...
        {sesiones.length === 0 ? (
          <div className="text-center py-12 text-gray-500"><div className="text-4xl mb-3">🏊</div><p>No hay sesiones esta semana. Planifica la primera.</p></div>
        ) : (
              ...
              <a key={s.id} href={'/sesion/' + s.id} className="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-orange-500 transition block">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    {s.usar_cronometro && <span className="text-xs bg-blue-900 text-blue-300 px-2 py-0.5 rounded-full">⏱ Cronometro</span>}
