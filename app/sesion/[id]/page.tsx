...
export default function PaginaSesion({ params }: { params: Promise<{ id: string }> }) {
  ...
  const crearTarea = async (e: React.FormEvent) => {
    ...
    else if (tipoMedicion === 'duracion' && tarea) await supabase.from('p_duracion').insert({ id_tarea: tarea.id, tiempo_planeado: Number(tiempo) })
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            {sesion.usar_cronometro && <span className="text-xs bg-blue-900 text-blue-300 px-2 py-0.5 rounded-full">⏱ Cronometro activado</span>}
          </div>
          <h2 className="text-2xl font-bold">{sesion.fecha_sesion}</h2>
          <p className="text-gray-400 text-sm mt-1">{sesion.duracion_minutos ? sesion.duracion_minutos + ' min planificados' : '—'} · RPE estimado: {sesion.rpe_estimado || '—'}</p>
        ...
        {sesion.estado !== 'Realizada' && (
          <div className="mb-6">
            {!sesionIniciada ? (
              <button onClick={iniciarSesion} className="w-full bg-green-600 hover:bg-green-700 text-white py-4 rounded-xl font-bold text-lg transition">
                ▶ Iniciar sesion
              </button>
            ) : (
              <div className="bg-gray-900 rounded-xl p-6 border border-green-500">
                ...
                <button onClick={finalizarSesion} className="w-full bg-orange-500 hover:bg-orange-600 text-white py-4 rounded-xl font-bold text-lg transition">
                  ✓ Finalizar sesion
                </button>
              </div>
            )}
        ...
        {sesion.estado === 'Realizada' && (
          <div className="bg-green-900 border border-green-500 rounded-xl p-4 mb-6 text-center">
            ...
            {sesion.duracion_real && <p className="text-green-400 text-sm">{sesion.duracion_real} minutos realizados</p>}
          </div>
        )}
        ...
        {mostrarPostSesion && (
          <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 rounded-xl p-6 w-full max-w-md border border-orange-500 max-h-screen overflow-y-auto">
              ...
              {sesion.usar_cronometro && <p className="text-green-400 text-sm mb-4">Duracion registrada: {formatTiempo(segundos)} ({Math.round(segundos/60)} min)</p>}
              <form onSubmit={guardarPostSesion} className="flex flex-col gap-4">
                <div className="bg-gray-800 rounded-xl p-4">
                  <div className="flex justify-between mb-2"><label className="text-white font-medium text-sm">RPE real (esfuerzo percibido)</label><span className="text-orange-400 font-bold">{rpeReal}/10</span></div>
                ...
                <div className="bg-gray-800 rounded-xl p-4">
                  <div className="flex justify-between mb-2"><label className="text-white font-medium text-sm">Dolor muscular post</label><span className="text-orange-400 font-bold">{dolorMuscular}/5</span></div>
                  ...
                  <div className="flex justify-between text-gray-500 text-xs mt-1"><span>Sin dolor</span><span>Mucho dolor</span></div>
                </div>
                ...
                <textarea placeholder="Notas sobre la sesion (opcional)" value={notasPost} onChange={e => setNotasPost(e.target.value)} className="bg-gray-800 text-white px-4 py-3 rounded-lg outline-none focus:ring-2 focus:ring-orange-500" rows={3} />
              ...
              <div key={t.id} className="bg-gray-900 rounded-xl p-5 border border-gray-800">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-300 text-sm">{t.series ? t.series + ' series' : ''}{t.series && t.descanso_segundos ? ' · ' + t.descanso_segundos + 's descanso' : ''}</p>
                    {t.comentario && <p className="text-gray-400 text-sm mt-1">{t.comentario}</p>}
                    {t.rpe_reportado && <p className="text-green-400 text-xs mt-2">✓ RPE real: {t.rpe_reportado}/10 · Sensacion: {t.sensacion_tecnica}/5</p>}
                  </div>
                </div>
              </div>
            ))}
...
ENDOFFILEcat > "app/sesion/[id]/page.tsx" << 'ENDOFFILE'
'use client'
import { useState, useEffect, use, useRef } from 'react'
import { supabase } from '@/lib/supabase'
