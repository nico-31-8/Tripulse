...
export default function PaginaTests({ params }: { params: Promise<{ id: string }> }) {
  ...
  const calcularVAM = () => {
    if (!velUltimo || !durTotal || !tiempoAguantado) return null
    const vam = Number(velUltimo) - (Number(incrementoVel) * (1 - Number(tiempoAguantado) / Number(durTotal)))
    return Math.round(vam * 10) / 10
  }

  const calcularCSS = () => {
    if (!tiempoGrande || !tiempoPequeno) return null
    const css = (Number(distGrande) - Number(distPequena)) / (Number(tiempoGrande) - Number(tiempoPequeno))
    return Math.round(css * 1000) / 1000
  }

  const calcularFTP = () => {
    if (!potenciaPico || !durEscalones || !tiempoNoCompletado) return null
    const ftp = (Number(potenciaPico) - Number(incrementoPot)) + (Number(incrementoPot) * Number(tiempoNoCompletado) / Number(durEscalones))
  ...
  const guardarTest1 = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    setError('')
    const vam = calcularVAM()
    const { error } = await supabase.from('test1_carrera').insert({
      id_deportista: Number(id),
      fecha,
      velocidad_ultimo_escalon: Number(velUltimo),
      duracion_total_escalon: Number(durTotal),
      tiempo_aguantado_ultimo: Number(tiempoAguantado),
      incremento_velocidad: Number(incrementoVel),
      vam
    })
  ...
  const guardarTest2 = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    setError('')
    const css = calcularCSS()
    const { error } = await supabase.from('test2_natacion').insert({
      id_deportista: Number(id),
      fecha,
      distancia_grande: Number(distGrande),
      distancia_pequena: Number(distPequena),
      tiempo_distancia_grande: Number(tiempoGrande),
      tiempo_distancia_pequena: Number(tiempoPequeno),
      css
    })
    if (error) { setError('Error: ' + error.message) }
    else { setMostrarForm(false); cargarDatos() }
  ...
  const guardarTest3 = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    setError('')
    const ftp = calcularFTP()
    const { error } = await supabase.from('test3_ciclismo').insert({
      id_deportista: Number(id),
      fecha,
      potencia_pico: Number(potenciaPico),
      tiempo_escalon_completado: Number(tiempoCompletado),
      tiempo_escalon_no_completado: Number(tiempoNoCompletado),
      duracion_escalones: Number(durEscalones),
      incremento_potencia: Number(incrementoPot),
      ftp
    })
    if (error) { setError('Error: ' + error.message) }
    else { setMostrarForm(false); cargarDatos() }
  ...
  const formatCSS = (css: number) => {
    const segundosPor100m = 100 / css
    const min = Math.floor(segundosPor100m / 60)
    const seg = Math.round(segundosPor100m % 60)
    return `${min}:${seg.toString().padStart(2, '0')} /100m`
  }

  const formatVAM = (vam: number) => {
    const segPorKm = 3600 / vam
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white">
      <nav className="bg-gray-900 px-6 py-4 flex justify-between items-center border-b border-gray-800">
        ...
        <a href={`/deportistas/${id}`} className="text-gray-400 hover:text-white text-sm transition">← Perfil deportista</a>
      </nav>
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {tab === 'carrera' && (
          tests1.length === 0 ? <div className="text-center py-12 text-gray-500"><div className="text-4xl mb-3">🏃</div><p>No hay tests de carrera todavia.</p></div> :
          <div className="grid gap-4">{tests1.map(t => <div key={t.id} className="bg-gray-900 rounded-xl p-5 border border-gray-800"><div className="flex justify-between"><div><p className="text-gray-400 text-sm">{t.fecha}</p><p className="font-bold text-lg text-orange-400">{t.vam} km/h VAM</p><p className="text-gray-300 text-sm">{formatVAM(t.vam)}</p></div></div></div>)}</div>
        )}
        {tab === 'natacion' && (
          tests2.length === 0 ? <div className="text-center py-12 text-gray-500"><div className="text-4xl mb-3">🏊</div><p>No hay tests de natacion todavia.</p></div> :
          <div className="grid gap-4">{tests2.map(t => <div key={t.id} className="bg-gray-900 rounded-xl p-5 border border-gray-800"><div className="flex justify-between"><div><p className="text-gray-400 text-sm">{t.fecha}</p><p className="font-bold text-lg text-orange-400">{t.css} m/s CSS</p><p className="text-gray-300 text-sm">{formatCSS(t.css)}</p></div></div></div>)}</div>
        )}
        {tab === 'ciclismo' && (
          tests3.length === 0 ? <div className="text-center py-12 text-gray-500"><div className="text-4xl mb-3">🚴</div><p>No hay tests de ciclismo todavia.</p></div> :
          <div className="grid gap-4">{tests3.map(t => <div key={t.id} className="bg-gray-900 rounded-xl p-5 border border-gray-800"><div className="flex justify-between"><div><p className="text-gray-400 text-sm">{t.fecha}</p><p className="font-bold text-lg text-orange-400">{t.ftp} W FTP</p></div></div></div>)}</div>
        )}
      </div>
    </main>
  )
